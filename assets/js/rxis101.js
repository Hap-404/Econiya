(() => {
  ...
  tabs.forEach((tab) => tab.addEventListener("click", () => {
    ...
    panels.forEach((panel) => panel.classList.toggle("is-active", panel.dataset.specPanel === key));
  }));
...
(() => {
  ...
  const reset = () => {
    ...
    pills.forEach((pill) => { pill.style.translate = ""; });
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const tabs = Array.from(
    tabList.querySelectorAll("[data-specification-tab]")
  );

  const panels = Array.from(
    document.querySelectorAll(
      "[data-specification-panel]"
    )
  );

  const activateTab = (selectedTab) => {
    const selectedName =
      selectedTab.dataset.specificationTab;
    ...
    panels.forEach((panel) => {
      const isSelected =
        panel.dataset.specificationPanel ===
        selectedName;
...
document.addEventListener("DOMContentLoaded", () => {
  const relatedSlider =
    document.querySelector(".rxis-related-slider");
